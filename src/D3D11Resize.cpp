#include "D3D11Processing/D3D11Resize.hpp"
...
struct D3D11Resizer::Pipelines { D3D11ComputePipeline resize; bool initialized = false; };
D3D11Resizer::D3D11Resizer() = default;
D3D11Resizer::~D3D11Resizer() = default;
D3D11Resizer::D3D11Resizer(D3D11Resizer&&) noexcept = default;
D3D11Resizer& D3D11Resizer::operator=(D3D11Resizer&&) noexcept = default;
void D3D11Resizer::Initialize(D3D11ProcessingContext& context) { m_context=&context; m_shaderCache.Initialize(context); m_pipelines.reset(); }
void D3D11Resizer::EnsureInitialized() const { if(!m_context) throw ValidationError("D3D11Resizer: resizer is not initialized"); }
void D3D11Resizer::EnsurePipelines(){ EnsureInitialized(); if(!m_pipelines) m_pipelines.reset(new Pipelines()); if(m_pipelines->initialized) return; m_pipelines->resize.Initialize(m_context->GetDevice(), m_shaderCache.GetComputeShader("ResizeRgba.hlsl")); m_pipelines->initialized=true; }
void D3D11Resizer::DispatchResize(ID3D11DeviceContext* deviceContext, D3D11Resource& src, D3D11Resource& dst, const ResizeDesc& desc){
    EnsurePipelines(); ValidateTexture2D(src,"D3D11Resizer::DispatchResize","src"); ValidateTexture2D(dst,"D3D11Resizer::DispatchResize","dst"); ValidateNotSameResource(src,dst,"D3D11Resizer::DispatchResize"); ValidateOutputUav(dst,"D3D11Resizer::DispatchResize");
    if(!IsRgbaLikeFormat(src.GetFormat()) || !IsRgbaLikeFormat(dst.GetFormat())) throw UnsupportedFormatError("D3D11Resizer::DispatchResize: only RGBA-like formats are supported");
    auto c = MakeResizeConstants(src,dst,desc);
    auto sv = CreateRgbaTextureViewSet(*m_context,src,true,false,src.GetFormat()); auto dv = CreateRgbaTextureViewSet(*m_context,dst,false,true,dst.GetFormat());
    ID3D11ShaderResourceView* srvs[] = { sv.srv.Get() }; ID3D11UnorderedAccessView* uavs[] = { dv.uav.Get() };
    BindAndDispatch(*m_context,m_pipelines->resize,deviceContext,srvs,1,uavs,1,c,c.dstWidth,c.dstHeight);
}
D3D11Resource D3D11Resizer::CreateOutputTexture(D3D11Core& core, UINT width, UINT height, DXGI_FORMAT format){
    EnsureInitialized(); if(width==0||height==0) throw ValidationError("D3D11Resizer::CreateOutputTexture: size is zero"); if(!IsRgbaLikeFormat(format)) throw UnsupportedFormatError("D3D11Resizer::CreateOutputTexture: only RGBA-like formats are supported");
