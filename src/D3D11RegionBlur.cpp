...
void ValidateRegionBlurDesc(const RegionBlurDesc& desc, const char* functionName) {
    ...
    if (desc.edgeSoftness < 0.0f) {
        throw ValidationError("D3D11RegionBlur::DispatchRegionBlur: edgeSoftness must be >= 0");
    }

    switch (desc.shape) {
    case RegionShape::Circle:
        if (!(desc.radius > 0.0f)) {
            throw ValidationError("D3D11RegionBlur::DispatchRegionBlur: radius must be > 0 for circle region");
        }
        break;
    case RegionShape::Rect:
        if (!(desc.rectWidth > 0.0f) || !(desc.rectHeight > 0.0f)) {
            throw ValidationError("D3D11RegionBlur::DispatchRegionBlur: rectWidth and rectHeight must be > 0 for rect region");
        }
        break;
    default:
        throw ValidationError("D3D11RegionBlur::DispatchRegionBlur: unsupported region shape");
    }
    ...
    case RegionSelection::Outside:
        break;
    default:
        throw ValidationError("D3D11RegionBlur::DispatchRegionBlur: unsupported region selection");
    }
}
...
void BindAndDispatchBlend(
    ...
    if (!deviceContext) {
        throw ValidationError("D3D11RegionBlur::DispatchRegionBlur: null device context");
    }
...
struct D3D11RegionBlur::Pipelines {
    D3D11ComputePipeline blend;
    bool initialized = false;
};

D3D11RegionBlur::D3D11RegionBlur() = default;
D3D11RegionBlur::~D3D11RegionBlur() = default;
D3D11RegionBlur::D3D11RegionBlur(D3D11RegionBlur&&) noexcept = default;
D3D11RegionBlur& D3D11RegionBlur::operator=(D3D11RegionBlur&&) noexcept = default;

void D3D11RegionBlur::Initialize(D3D11ProcessingContext& context) {
    m_context = &context;
...
void D3D11RegionBlur::EnsureInitialized() const {
    if (!m_context) {
        throw ValidationError("D3D11RegionBlur: processor is not initialized");
    }
}

void D3D11RegionBlur::EnsurePipelines() {
    EnsureInitialized();
...
void D3D11RegionBlur::DispatchRegionBlur(
    ID3D11DeviceContext* deviceContext,
    D3D11Resource& src,
    ...
    constexpr const char* kFunction = "D3D11RegionBlur::DispatchRegionBlur";
    ...
    ValidateHasSrv(blurScratch, kFunction, "blurScratch");
    ValidateHasUav(blurScratch, kFunction, "blurScratch");
    ValidateHasSrv(blurred, kFunction, "blurred");
    ValidateHasUav(blurred, kFunction, "blurred");
    ValidateHasUav(dst, kFunction, "dst");

    const DXGI_FORMAT srcFormat = ResolveFormat(desc.srcFormat, src);
    const DXGI_FORMAT blurredFormat = ResolveFormat(DXGI_FORMAT_UNKNOWN, blurred);
    const DXGI_FORMAT dstFormat = ResolveFormat(desc.dstFormat, dst);

    if (!IsRgbaLikeFormat(srcFormat) || !IsRgbaLikeFormat(blurredFormat) || !IsRgbaLikeFormat(dstFormat)) {
        throw UnsupportedFormatError("D3D11RegionBlur::DispatchRegionBlur: only RGBA-like formats are supported");
    }

    ValidateRgbaUavSupport(*m_context, blurScratch.GetFormat(), kFunction);
    ...
    const auto blurredDesc = blurred.GetTexture2DDesc();
    ...
    if (srcRect.width != dstRect.width || srcRect.height != dstRect.height) {
        throw ValidationError("D3D11RegionBlur::DispatchRegionBlur: region blur does not resize; srcRect and dstRect sizes must match");
    }

    if (blurredDesc.Width < dstRect.width || blurredDesc.Height < dstRect.height) {
        throw ValidationError("D3D11RegionBlur::DispatchRegionBlur: blurred texture is smaller than processing rect");
    }
    ...
    std::copy(desc.blurBorderColor, desc.blurBorderColor + 4, blurDesc.borderColor);

    m_blurrer.DispatchBlur(deviceContext, src, blurScratch, blurred, blurDesc);

    auto srcView = CreateRgbaTextureViewSet(*m_context, src, true, false, srcFormat);
    auto blurredView = CreateRgbaTextureViewSet(*m_context, blurred, true, false, blurredFormat);
...
D3D11Resource D3D11RegionBlur::CreateOutputTexture(
    D3D11Core& core,
    UINT width,
    UINT height,
    DXGI_FORMAT format) {
    ...
    if (width == 0 || height == 0) {
        throw ValidationError("D3D11RegionBlur::CreateOutputTexture: size is zero");
    }

    if (!IsRgbaLikeFormat(format)) {
        throw UnsupportedFormatError("D3D11RegionBlur::CreateOutputTexture: only RGBA-like formats are supported");
    }

    ValidateRgbaUavSupport(*m_context, format, "D3D11RegionBlur::CreateOutputTexture");
...
D3D11Resource D3D11RegionBlur::CreateScratchTexture(
    D3D11Core& core,
    UINT width,
    UINT height,
    DXGI_FORMAT format) {
    ...
    if (width == 0 || height == 0) {
        throw ValidationError("D3D11RegionBlur::CreateScratchTexture: size is zero");
    }

    if (!IsRgbaLikeFormat(format)) {
        throw UnsupportedFormatError("D3D11RegionBlur::CreateScratchTexture: only RGBA-like formats are supported");
    }

    ValidateRgbaUavSupport(*m_context, format, "D3D11RegionBlur::CreateScratchTexture");
...
D3D11Resource D3D11RegionBlur::CreateBlurredTexture(
    D3D11Core& core,
    UINT width,
    UINT height,
    DXGI_FORMAT format) {
    ...
    if (width == 0 || height == 0) {
        throw ValidationError("D3D11RegionBlur::CreateBlurredTexture: size is zero");
    }

    if (!IsRgbaLikeFormat(format)) {
        throw UnsupportedFormatError("D3D11RegionBlur::CreateBlurredTexture: only RGBA-like formats are supported");
    }

    ValidateRgbaUavSupport(*m_context, format, "D3D11RegionBlur::CreateBlurredTexture");
