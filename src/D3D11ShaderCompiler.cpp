...
ShaderBytecode CompileShaderFromSource_Dxc(
    ...
    D3D11CORE_THROW_IF_FAILED(DxcCreateInstance(CLSID_DxcCompiler,
                                                IID_PPV_ARGS(&compiler)));
    D3D11CORE_THROW_IF_FAILED(utils->CreateDefaultIncludeHandler(&includeHandler));
    ...
    D3D11CORE_THROW_IF_FAILED(result->GetOutput(DXC_OUT_OBJECT,
                                                IID_PPV_ARGS(&blob), nullptr));
    if (!blob || blob->GetBufferSize() == 0) {
        throw std::runtime_error("DXC: no output");
    }
