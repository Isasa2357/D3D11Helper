#include "D3D11Processing/D3D11FusedPipeline.hpp"
...
struct D3D11FusedProcessor::Pipelines { D3D11ComputePipeline rgbToRgbResize; D3D11ComputePipeline yuv420ToRgbResize; bool initialized = false; };
D3D11FusedProcessor::D3D11FusedProcessor() = default;
D3D11FusedProcessor::~D3D11FusedProcessor() = default;
D3D11FusedProcessor::D3D11FusedProcessor(D3D11FusedProcessor&&) noexcept = default;
D3D11FusedProcessor& D3D11FusedProcessor::operator=(D3D11FusedProcessor&&) noexcept = default;
void D3D11FusedProcessor::Initialize(D3D11ProcessingContext& context){m_context=&context;m_shaderCache.Initialize(context);m_pipelines.reset();}
void D3D11FusedProcessor::EnsureInitialized() const { if(!m_context) throw ValidationError("D3D11FusedProcessor: processor is not initialized"); }
void D3D11FusedProcessor::EnsurePipelines(){EnsureInitialized(); if(!m_pipelines)m_pipelines.reset(new Pipelines()); if(m_pipelines->initialized)return; m_pipelines->rgbToRgbResize.Initialize(m_context->GetDevice(),m_shaderCache.GetComputeShader("FusedRgbToRgbResize.hlsl")); m_pipelines->yuv420ToRgbResize.Initialize(m_context->GetDevice(),m_shaderCache.GetComputeShader("FusedYuv420ToRgbResize.hlsl")); m_pipelines->initialized=true;}
void D3D11FusedProcessor::DispatchConvertResize(ID3D11DeviceContext* deviceContext, D3D11Resource& src, D3D11Resource& dst, const FusedConvertResizeDesc& desc){
    EnsurePipelines(); ValidateTexture2D(src,"D3D11FusedProcessor::DispatchConvertResize","src"); ValidateTexture2D(dst,"D3D11FusedProcessor::DispatchConvertResize","dst"); ValidateNotSameResource(src,dst,"D3D11FusedProcessor::DispatchConvertResize"); ValidateOutputUav(dst,"D3D11FusedProcessor::DispatchConvertResize");
    auto c=MakeFusedConstants(src,dst,desc); const DXGI_FORMAT sf=(DXGI_FORMAT)c.srcFormat; const DXGI_FORMAT df=(DXGI_FORMAT)c.dstFormat;
    if(IsRgbaLikeFormat(sf)&&IsRgbaLikeFormat(df)){
        auto sv=CreateRgbaTextureViewSet(*m_context,src,true,false,sf); auto dv=CreateRgbaTextureViewSet(*m_context,dst,false,true,df); ID3D11ShaderResourceView* srvs[]={sv.srv.Get()}; ID3D11UnorderedAccessView* uavs[]={dv.uav.Get()}; BindAndDispatch(*m_context,m_pipelines->rgbToRgbResize,deviceContext,srvs,1,uavs,1,c,c.dstWidth,c.dstHeight);
    } else if(IsYuv420Format(sf)&&IsRgbaLikeFormat(df)){
        ValidateYuv420Rect(ResolveRect(desc.srcRect,src.GetWidth(),src.GetHeight()),"D3D11FusedProcessor::DispatchConvertResize","srcRect");
        auto sv=CreateYuv420SrvViewSet(*m_context,src); auto dv=CreateRgbaTextureViewSet(*m_context,dst,false,true,df); ID3D11ShaderResourceView* srvs[]={sv.ySrv.Get(),sv.uvSrv.Get()}; ID3D11UnorderedAccessView* uavs[]={dv.uav.Get()}; BindAndDispatch(*m_context,m_pipelines->yuv420ToRgbResize,deviceContext,srvs,2,uavs,1,c,c.dstWidth,c.dstHeight);
    } else {
        throw UnsupportedFormatError("D3D11FusedProcessor::DispatchConvertResize: supported conversions are RGBA-like -> RGBA-like and NV12/P010 -> RGBA-like");
    }
}
D3D11Resource D3D11FusedProcessor::CreateOutputTexture(D3D11Core& core, UINT width, UINT height, DXGI_FORMAT format){EnsureInitialized(); if(width==0||height==0) throw ValidationError("D3D11FusedProcessor::CreateOutputTexture: size is zero"); if(!IsRgbaLikeFormat(format)) throw UnsupportedFormatError("D3D11FusedProcessor::CreateOutputTexture: only RGBA-like output formats are supported"); if(format==DXGI_FORMAT_R8G8B8A8_UNORM && !m_context->SupportsRgba8Uav()) throw UnsupportedFeatureError("R8G8B8A8 UAV is not supported"); if(format==DXGI_FORMAT_B8G8R8A8_UNORM && !m_context->SupportsBgra8Uav()) throw UnsupportedFeatureError("B8G8R8A8 UAV is not supported"); if(format==DXGI_FORMAT_R16G16B16A16_FLOAT && !m_context->SupportsRgba16FloatUav()) throw UnsupportedFeatureError("RGBA16F UAV is not supported"); return CreateTexture2D(core,width,height,format,D3D11_BIND_SHADER_RESOURCE|D3D11_BIND_UNORDERED_ACCESS);}
} }
