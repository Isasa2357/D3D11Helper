...
ComPtr<ID3D11ShaderReflection> CreateReflection(const void* bytecode, size_t bytecodeSize) {
    ...
    if (FAILED(hr) || !reflection) {
        throw std::runtime_error(
            ...
            "D3D11Helper shader reflection supports D3DCompile/DXBC bytecode.");
    }
    return reflection;
}

ShaderSignatureParameterInfo ConvertSignatureParameter(const D3D11_SIGNATURE_PARAMETER_DESC& desc) {
...
ShaderReflectionInfo ReflectShaderBytecode(const void* bytecode, size_t bytecodeSize) {
    ...
    D3D11CORE_THROW_IF_FAILED(reflection->GetDesc(&shaderDesc));
    ...
    for (UINT i = 0; i < shaderDesc.BoundResources; ++i) {
        ...
        D3D11CORE_THROW_IF_FAILED(reflection->GetResourceBindingDesc(i, &bindDesc));
    ...
    for (UINT i = 0; i < shaderDesc.ConstantBuffers; ++i) {
        ...
        D3D11CORE_THROW_IF_FAILED(cb->GetDesc(&bufferDesc));
        ...
        for (UINT v = 0; v < bufferDesc.Variables; ++v) {
            ...
            D3D11CORE_THROW_IF_FAILED(variable->GetDesc(&variableDesc));
    ...
    for (UINT i = 0; i < shaderDesc.InputParameters; ++i) {
        ...
        D3D11CORE_THROW_IF_FAILED(reflection->GetInputParameterDesc(i, &paramDesc));
    ...
    for (UINT i = 0; i < shaderDesc.OutputParameters; ++i) {
        D3D11_SIGNATURE_PARAMETER_DESC paramDesc = {};
        D3D11CORE_THROW_IF_FAILED(reflection->GetOutputParameterDesc(i, &paramDesc));
...
std::vector<D3D11_INPUT_ELEMENT_DESC> MakeD3D11InputElementDescs(
