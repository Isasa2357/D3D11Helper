...
**Direct3D 11** の定型処理を薄くラップした **Layer 1 / Layer 2 / Layer 3 構成の C++17 ヘルパライブラリ**です。

デバイス生成、Immediate / Deferred Context、DXGI アダプタ選択、リソース作成、View 作成、Compute / Graphics パイプライン、シェーダコンパイル、ステージングバッファ、共有リソース、D3D11.4 Fence、GPU 画像処理（format conversion / resize / remap / composite / fused convert+resize）など、D3D11 アプリケーションで繰り返し書くコードを自由関数と軽量クラスにまとめています。
...
- **3 レイヤー分離** — Layer 1（Core: Device / Context / DXGI）、Layer 2（Framework: Resource / View / Pipeline）、Layer 3（Processing: GPU 画像処理）を分離。
- **`D3D11Core&` を取る自由関数** — 1 行呼ぶだけでバッファ / テクスチャ / View / Sampler を作れる。
...
- **D3D11Processing** — GPU 上で format conversion、resize、remap、composite、fused convert+resize を実行する Layer 3。NV12 / P010 / RGBA8 / BGRA8 / RGBA16F を扱う。
...
|      D3D11FormatConverter / D3D11Resizer                     |
|      D3D11Remapper / D3D11Compositor                         |
|      D3D11FusedProcessor                                     |
...
`D3D11Core` は、D3D11 の device / immediate context / DXGI を束ねるファサードです。アプリケーション内では 1 つの `std::shared_ptr<D3D11Core>` を作成し、Renderer、Camera、Processing など複数のサブシステムで共有する想定です。

D3D12Helper と異なり、D3D11 では command queue / command list / resource barrier / descriptor heap を明示管理しません。Layer 3 の Processing も `ID3D11DeviceContext*` に対して `Dispatch*` で compute dispatch を発行します。
...
│   └── D3D11Processing/            # Layer 3 ヘッダ
├── shaders/D3D11Processing/        # Processing Layer HLSL / hlsli
├── src/                            # 実装 (.cpp)
├── sample/                         # サンプルコード
│   ├── 01_HelloDevice/
│   ├── 02_ComputeGrayscale/
...
│   ├── 05_DynamicStreaming/
│   ├── 06_SharedResource/
│   ├── 07_ProcessingFusedConvertResize/
│   └── 08_ProcessingP010Rgba16/
├── Test/                           # テスト
└── doc/                            # API リファレンス・使用パターン
```

ライブラリ本体は `include/` と `src/` に分離しています。Processing Layer を使う場合は、`shaders/D3D11Processing/` も実行時に参照できる場所へ配置してください。
...
Processing Layer の最小形は次のようになります。
...
| 07 | ProcessingFusedConvertResize | NV12 → RGBA resize を fused dispatch で実行し、CPU readback で検証 | 不要 |
...
`Test/` には、既存の standalone tests と、`TEST(Suite, Case)` 形式の harness tests が入っています。
...
| `Record*` Processing API | `Dispatch*` Processing API | D3D11 は context に即時発行 |
...
| [`doc/D3D11Processing.md`](doc/D3D11Processing.md) | Layer 3 Processing API リファレンス |
