...
struct PyramidBlurDesc {
    ...
    // Number of 2x downsample steps before blurring. src/dst rect sizes must be divisible by 2^levels.
    UINT levels = 2;
...
struct PyramidRegionBlurDesc {
    ...
    // Pyramid blur settings. src/dst rect sizes must be divisible by 2^levels.
    UINT levels = 2;
    BlurMode blurMode = BlurMode::Gaussian;
    UINT blurRadius = 5;
    float blurSigma = 2.0f;
