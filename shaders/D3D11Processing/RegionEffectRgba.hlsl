...
static const uint REGION_EFFECT_COPY = 0;
static const uint REGION_EFFECT_DARKEN = 1;
static const uint REGION_EFFECT_TINT = 2;
static const uint REGION_EFFECT_GRAYSCALE = 3;
static const uint REGION_EFFECT_HIGHLIGHT = 4;
static const uint REGION_EFFECT_ALPHA_FADE = 5;
static const uint REGION_EFFECT_VIGNETTE = 6;
...
float4 ApplyRegionEffect(float4 color, float mask)
{
    mask = saturate(mask);

    if (Effect == REGION_EFFECT_COPY) {
        return color;
    }

    if (Effect == REGION_EFFECT_DARKEN) {
        color.rgb *= lerp(1.0f, DarkenFactor, mask);
        return saturate(color);
    }

    if (Effect == REGION_EFFECT_TINT) {
        color.rgb = lerp(color.rgb, TintColor.rgb, saturate(TintStrength * mask));
        return saturate(color);
    }

    if (Effect == REGION_EFFECT_GRAYSCALE) {
        float gray = dot(color.rgb, float3(0.2126f, 0.7152f, 0.0722f));
        color.rgb = lerp(color.rgb, float3(gray, gray, gray), saturate(GrayscaleStrength * mask));
        return saturate(color);
    }

    if (Effect == REGION_EFFECT_HIGHLIGHT) {
        color.rgb = lerp(color.rgb, saturate(color.rgb + HighlightColor.rgb * HighlightStrength), mask);
        return saturate(color);
    }

    if (Effect == REGION_EFFECT_ALPHA_FADE) {
        color.a *= lerp(1.0f, AlphaFactor, mask);
        return saturate(color);
    }

    if (Effect == REGION_EFFECT_VIGNETTE) {
        float vignetteMask = saturate(mask * VignetteStrength);
        color.rgb *= lerp(1.0f, DarkenFactor, vignetteMask);
        return saturate(color);
    }
...
void main(uint3 tid : SV_DispatchThreadID)
{
    ...
    float4 color = Src.Load(int3((int)srcPos.x, (int)srcPos.y, 0));
    float mask = RegionMask(dstCenter);
    Dst[dstPos] = ApplyRegionEffect(color, mask);
}
